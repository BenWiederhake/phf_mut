...
impl<H: HasherInverse> Set<H> {
    pub fn iter<'a>(&'a self) -> impl Iterator<Item=H::K> {
        (0..self.backing.len()).filter_map(|index| {
            if self.has(index) {
                Some(self.hash.invert(index))
            } else {
                None
            }
        })
...
impl<'a, H: HasherInverse> IntoIterator for &'a Set<H> {
    type Item = H::K;
    type IntoIter = impl Iterator<Item=H::K>;
...
impl<'a, H: HasherInverse> Iterator for SetIter<'a, H> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.next == size {
            println!("End outside at {}", self.next);
            None
        } else {
            println!("End at some {}", self.next);
